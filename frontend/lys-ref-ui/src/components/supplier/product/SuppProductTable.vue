<template>
  ...
  >
    <template #top>
      ...
      <l-dt-top :ax="axToUse" :title="props.title ?? $t('supplier_products.title')" :headers="headers" :excelDlUrl="excelDlUrl" 
      ...
      <v-row density="comfortable">
        <v-col class="mb-2">
          <div class="dt-subtitle" v-if="props.internal">{{ $t('internal_view.supplier_products.p1') }}</div>
          <div class="dt-subtitle" v-else>{{ $t('tenant_view.supplier_products.p1') }}</div>
        </v-col>
      </v-row>
    </template>
