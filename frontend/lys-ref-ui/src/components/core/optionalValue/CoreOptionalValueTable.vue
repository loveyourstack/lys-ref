<template>
  ...
  <l-dialog-card v-model="showImport">
    <l-text-array-entry :title="$t('optional_values.import_enter')" :saving="importing" :max-items="maxImportItems" :enterDisabled="!auth.isWriter()"
  ...
  >
    <template #top>
      <l-dt-top :ax="ax" :title="props.title ?? $t('optional_values.title')" :headers="headers" :excelDlUrl="excelDlUrl" 
      ...
      <v-row density="comfortable">
        <v-col class="mb-2">
          <div class="dt-subtitle">{{ $t('optional_values.p1') }}</div>
        </v-col>
      </v-row>
    </template>
