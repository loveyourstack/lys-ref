...
func EqualCmd(cliApp *cliapp.App) *cobra.Command {
	return &cobra.Command{
		Use:   "eq",
		Short: "Generates Go Equal function from the supplied schema + table",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			if err != nil {
				return fmt.Errorf("lysgen.Equal failed: %w", err)
			}
