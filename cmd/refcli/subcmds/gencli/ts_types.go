...
func TsTypesCmd(cliApp *cliapp.App) *cobra.Command {
	return &cobra.Command{
		Use:   "ts",
		Short: "Generates Typescript interfaces and constructor funcs from the supplied schema + table",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			res, err := lysgen.TsTypes(cmd.Context(), ownerDb, args[0], args[1])
			if err != nil {
				return fmt.Errorf("lysgen.TsTypes failed: %w", err)
			}
