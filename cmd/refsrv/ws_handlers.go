...
func (srvApp *httpServerApplication) wsNotificationsRegister(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		lys.HandleInternalError(ctx, fmt.Errorf("wsNotificationsRegister: user not authenticated"), srvApp.Logger, w)
		return
	}
	...
	if err := srvApp.NotificationHub.ServeUserSocket(ctx, w, r, userInfo.UserId); err != nil {
		srvApp.Logger.Error("wsNotificationsRegister: srvApp.NotificationHub.ServeUserSocket failed", "user_id", userInfo.UserId, "error", err)
	}
}
