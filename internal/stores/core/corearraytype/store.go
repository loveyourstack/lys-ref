...
const (
	name           string = "Array types"
	schemaName     string = "core"
	tableName      string = "array_type"
	viewName       string = "array_type"
	pkColName      string = "id"
	defaultOrderBy string = "id DESC"
)

type Input struct {
	// don't use omitempty for array types. UI expects empty array rather than field omission
	CBool    []bool         `db:"c_bool" json:"c_bool"`
	CDate    []lystype.Date `db:"c_date" json:"c_date"`
	CEnum    []string       `db:"c_enum" json:"c_enum"`
